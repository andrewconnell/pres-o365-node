 ...
 */
gulp.task('gen-client-tsrefs', function () {
  var target  = gulp.src(config.clientTsDefList);
  var sources = gulp.src([config.allClientTypeScript], {read: false});
...
gulp.task('compile-client-ts', function () {
  var sourceTsFiles = [config.allClientTypeScript,  // path to typescript files
    ...
    config.customTsDefList,                         // reference to custom.d.ts file
    config.clientTsDefList];                        // reference to apps.d.ts file
  ...
  tsResult.dts.pipe(gulp.dest(config.clientApp));
  return tsResult.js
...
gulp.task('watch', function () {
  gulp.watch([config.allTypeScript], ['lint', 'gen-client-tsrefs', 'gen-server-tsrefs', 'compile-client-ts', 'compile-server-ts']);
});
...
gulp.task('watch-nodemon', function () {
  ...
  nodemon({
    ...
    tasks  : ['lint', 'gen-client-tsrefs', 'gen-server-tsrefs', 'compile-client-ts', 'compile-server-ts']
...
/* default gulp task */
gulp.task('default', ['lint', 'gen-client-tsrefs', 'gen-server-tsrefs', 'compile-client-ts', 'compile-server-ts', 'watch']);
